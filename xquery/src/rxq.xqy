...
declare variable $rxq:default-endpoint as xs:string := "/url-rewriter.xqy?mode=" ||  $rxq:_MUX_MODE;
...
declare function rxq:response($body as item()*, $not-found-message as xs:string) {
  if (fn:exists($body)) then response($body, 200, "OK")
  else response("[mldeploy] " || $not-found-message, 404, "Not Found")
};

declare %private function rxq:response($body as item()*, $status-code as xs:int, $status-msg as xs:string) {
  xdmp:set-response-code($status-code, $status-msg),
  $body
};


declare function rxq:error-response($msg as xs:string) {
  xdmp:set-response-code(500, $msg),
  $msg
};
